...
其中 $s$ 是非固体邻居的数量。密度漂移补偿通过 `compensate_drift` 选项控制，将粒子密度偏差纳入散度修正。补偿采用单向策略：仅对密度高于全局目标的单元施加膨胀修正（系数 0.5），密度低于目标的单元不施加压缩修正。这种设计防止了粒子过度聚集，同时避免了双向补偿引发的反馈环路。全局目标密度在仿真开始时一次性计算，不随帧更新，为长时间模拟提供一致的密度基准。
...
> 遗憾的是，欧拉方法最终依旧没能解决粒子流失以及不扩散的问题。因此这里没有提供demo。
