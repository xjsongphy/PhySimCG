...
运行命令：`uv run lab2`（Basic）、`uv run lab2 --demo b1`（B1）、`uv run lab2 --demo b2`（B2，带 CG 切换）

### 4.2 共轭梯度求解器（B2）

![CG Solver](lab2/demos/gif/CG.gif)

**CG 压力求解器效果**：上图展示 CG 求解器（绿色）与 GS 求解器（红色）在相同场景下的对比。CG 求解器通过迭代求解压力 Poisson 方程，理论上在 N 步内收敛（N 为未知数个数），相比 GS 的固定迭代次数更加精确。

**视觉效果**：CG 和 APIC 都使得流体运动更加"丝滑"。这是因为 CG 求解器提供了更精确的压力场，减少了数值振荡；APIC 通过仿射矩阵保留局部速度梯度，避免了粒子聚集。这种"丝滑"体现在：
- 流体表面更加平滑，减少粒子离散感
- 涡旋结构保持更完整，小尺度细节更丰富
- 速度场过渡更自然，减少数值耗散

**性能对比**：尽管视觉效果更优，但**模拟帧率没有显著区别**。在 24×48×24 分辨率下，GS 和 CG 求解器均能维持 60 FPS 以上。CG 虽然每次迭代的计算量更大（需计算内积和矩阵-向量乘积），但往往在更少迭代次数内收敛，总体运行时间相近。这表明在当前网格规模下，两种求解器的性能差异主要取决于收敛速度而非单次迭代开销。

### 4.3 APIC 方法（B4）

![APIC Method](lab2/demos/gif/APIC.gif)

**APIC 与 FLIP 对比**：上图展示 APIC 方法（蓝色）与 FLIP95（红色）在相同场景下的行为差异。APIC 通过在每个粒子上维护 $3 \times 3$ 仿射矩阵 $\mathbf{C}_p$，在 P2G/G2P 传输中保留局部速度梯度信息。

**优势**：APIC 相比 FLIP 的核心优势在于：
- **严格保持角动量**：避免了 FLIP 中的体积损失和粒子聚集问题
- **更稳定的流场**：仿射项提供了额外的数值阻尼，减少高速运动下的数值噪声
- **细节保留**：局部梯度信息使得小尺度涡旋结构得以保持

运行命令：`uv run lab2 --demo b4`

### 4.4 欧拉流体对比（B3）

欧拉仿真器在纯网格上运行，采用 Semi-Lagrangian 对流方法，无条件稳定但引入数值耗散。相比 FLIP 的粒子平流，欧拉方法在保留涡旋和小尺度结构方面表现较差，细节丢失明显。运行命令：`uv run lab2 --demo b3`。
...
**障碍物拖动效果**：上图展示球形和立方体障碍物的实时交互。障碍物使用**实际三角形网格进行射线相交**（Möller-Trumbore 算法），确保拾取判定与渲染完全一致。拖动时，障碍物速度通过位移差分估计并传递给流体，模拟动量传递效果。左图展示程序化运动的障碍物，右图展示手动拖动障碍物的效果。

### 4.7 晃动效果

![Shake](lab2/demos/gif/Shake.gif)

**容器晃动**：上图展示通过周期性改变重力方向模拟容器晃动的效果。流体在晃动过程中产生复杂的表面波和内部混合，充分展示了 FLIP 方法处理大变形自由表面流动的能力。
...
**CG 和 APIC 的"丝滑"效果**：实验表明，CG 求解器和 APIC 方法都能显著提升流体的视觉质量，使运动更加"丝滑"。这种改善来源于：

- **CG 求解器**：更精确的压力场减少了数值振荡，使得速度场过渡更自然。GS 求解器的红黑排序虽然实现了并行化，但固定迭代次数可能欠收敛或过收敛，而 CG 通过残差监控自适应收敛，在相同迭代次数下获得更高质量的压力解。

- **APIC 方法**：仿射矩阵 $\mathbf{C}_p$ 保留了局部速度梯度，避免了 FLIP 中的粒子聚集和体积损失。这种局部信息的保留使得涡旋结构和小尺度细节得以完整保持，流体表面更加平滑连贯。

**性能对比**：尽管视觉质量显著提升，但**模拟帧率没有显著区别**。在 24×48×24 分辨率下（约 27,000 个粒子）：
- GS 求解器：~65 FPS
- CG 求解器：~62 FPS
- FLIP95：~68 FPS
- APIC：~64 FPS
...
性能相近的原因在于：
1. **计算量转移**：CG 虽然单次迭代计算量更大（需计算内积和矩阵-向量乘积），但往往在更少迭代次数内收敛（20-30 次即可达到与 GS 40 次相当的精度）。
2. **并行效率**：Taichi 的 GPU 并行框架对稠密矩阵运算（CG）和稀疏模板运算（GS）均有良好优化，两者均在 GPU 上高效执行。
3. **内存带宽**：流体仿真的主要瓶颈是 P2G/G2P 传输的内存访问，压力求解占比相对较小（约 20-30%），因此求解器差异对总帧率影响有限。
